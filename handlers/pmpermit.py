    ...
    if PMPERMIT == "ENABLE":
        if PMSET:
            ...
            if chat_id in pchats:
                return
            await USER.send_message(message.chat.id, f"**Hallo Saya assistant dari [{BOT_NAME}](https://t me/{BOT_USERNAME}).**\n\n🔴 Note:\n\n༄ Jangan spam agar bot tidak lag.\n༄ Jangan post apapun melalui private\n༄ Mohon kirimkan tautan grupmu agar saya bisa bergabung\n\n⨀ Updates : @{UPDATES_CHANNEL} \n⨀ Support : @{GROUP_SUPPORT}\n👩‍💻 Dev : @{OWNER_NAME}\n\n")
            return
...
async def pmPermiat(client: USER, message: Message):
    ...
    if not chat_id in pchats:
        pchats.append(chat_id)
        await message.reply_text("✅ menyetujui untuk pm.")
        return
    message.continue_propagation()    
...
async def rmpmPermiat(client: USER, message: Message):
    ...
    if chat_id in pchats:
        ...
        await message.reply_text("❌ menolak untuk pm.")
        return
    message.continue_propagation()
